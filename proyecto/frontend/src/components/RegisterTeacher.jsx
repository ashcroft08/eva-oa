...
export const RegisterTeacher = () => {
  const { users, getUsersTeacher, getUser, updateUser, deleteUser } = useUser();
  ...
  useEffect(() => {
    ...
    fetchUsers();
  }, [getUsersTeacher]); // Solo se ejecuta una vez al montar el componente
  ...
  const { signup, errors: registerErrors } = useAuth(); // Obtén los errores y la función setErrors

  const onSubmit = async (values) => {
    console.log("Datos enviados:", values);
    const success = await signup(values);
    if (success) {
      CustomToast("¡Docente registrado exitosamente!", "success");
      setVisible(false); // Cierra el modal
      await getUsersTeacher(); // Actualiza la lista de usuarios después de registrar uno nuevo
  ...
  const onSubmitEdit = async (values) => {
    try {
      ...
      CustomToast("¡Docente actualizado exitosamente!", "success");
      setEditVisible(false);
      await getUsersTeacher(); // Actualizar lista
    } catch (error) {
      CustomToast(
